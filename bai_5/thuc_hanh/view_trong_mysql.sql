...
-- Trong câu lệnh SELECT định nghĩa view 
-- không được sử dụng từ khoá DISTINCT, TOP, GROUP BY, HAVING, ORDER_BY và UNION.
...
-- phải là các cột trong các bảng cơ sở. 
-- Trong danh sách chọn không được chứa các biểu thức tính toán, các hàm gộp.
-- Các cột được ước lượng không thể bị cập nhật.
-- Tất cả các cột NOT NULL từ bảng ban đầu 
-- phải được bao trong view để cho truy vấn INSERT vận hành.
-- Khi view của bạn thỏa mãn tất cả các quy tắc trên 
-- thì bạn có thể cập nhật view bằng câu lệnh:
...
-- Xoá view
-- Khi bạn đã có 1 view, nhưng khi không sử dụng đến nó nữa, 
-- thì chúng ta nên xóa view đó đi. cú pháp để xóa view như sau: 
-- DROP VIEW view_name;
